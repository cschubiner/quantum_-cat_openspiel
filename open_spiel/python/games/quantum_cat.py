...
class QuantumCatGameState(pyspiel.State):
  ...
  Phases:
    0) Dealing (chance)
    1) Discard (each player discards 1 card face-down)
    2) Prediction (must pick 1..4)
    3) Trick-taking (num_tricks)
    4) Scoring (terminal)

  Adjacency board => _board_ownership[color][rank] = player_id or -1 if empty.
  If you match your bid and did NOT paradox, you get adjacency bonus = size of
  your largest connected cluster of squares on the board.

  Paradox occurs if you have no legal moves on your turn in the trick-taking phase.
  Your final score is then -(#tricks_won), and you forfeit adjacency bonus.
  """
  ...
  def __init__(self, game: QuantumCatGame):
    ...
    # Discard tracking (face-down for others, but known in the real state)
    self._has_discarded = [False] * self._num_players
    # If -1 => not yet set. If in [1..max_card_value], that rank was discarded.
    ...
    # NEW: Keep record of completed tricks to know how many cards each player has played.
    # We'll store each completed trick as a list of (player_id, (rank_val, color_str)) tuples,
    # or simply replicate the final state of _cards_played_this_trick if that’s easier.
  ...
  def _apply_discard(self, action):
    ...
    self._discarded_cards[player] = action + 1  # store the rank
  ...
  def _advance_prediction_phase(self):
    ...
    if all(x >= 1 for x in self._predictions):  # everyone predicted 1..4
  ...
  def _trick_legal_actions(self, player):
    ...
    A legal action is any color-rank combination not yet claimed, for which
    the player still has that rank in hand and a token for that color.
    Additionally, if the trick hasn't led color yet and trump isn't broken,
    you can't lead red unless it's your only color option.
    """
    hand_vec = self._hands[player]
    actions = []
    for rank_idx in range(self._num_card_types):
    ...
    if self._led_color is None and not self._trump_broken:
      # Remove all red leads unless it's the only possibility
      red_actions = [a for a in actions if (a // self._num_card_types) == 0]
      if red_actions and len(actions) > len(red_actions):
        # We have non-trump leads available, so we remove the red ones
  ...
  def _apply_trick_action(self, action):
    ...
    # If this is not the lead color and the chosen color is "R", we break trump
    ...
    if self._led_color is None:
      ...
      # If player did not follow lead color => remove that token
    ...
    if self._current_player == self._start_player:
      ...
      # Store the completed trick for info. We'll copy the array so we can reset it
      completed_trick_snapshot = []
      for p_idx, cardinfo in enumerate(self._cards_played_this_trick):
        completed_trick_snapshot.append((p_idx, cardinfo))  # (player, (rank, color))
      self._completed_tricks.append(completed_trick_snapshot)
  ...
  def _compute_final_scores(self):
    ...
    for p in range(self._num_players):
      ...
      if self._has_paradoxed[p]:
        # If paradox => score = - (tricks) [and no adjacency bonus]
        raw_scores[p] = -float(tricks)
      else:
        # If not paradoxed => normal scoring
        base = float(tricks)
    ...
    for c_idx in range(self._num_colors):
      for r_idx in range(self._num_card_types):
        if (self._board_ownership[c_idx][r_idx] == player) and (not visited[c_idx][r_idx]):
          # BFS/DFS to find connected cluster
          size = 0
  ...
  # -------------------------------------------------------------------
  # Counting how many cards a player has played so far
  # -------------------------------------------------------------------
  def _count_cards_played_by(self, player_id):
    """
    Number of cards (trick plays) made by 'player_id' so far,
    including completed tricks and the current trick.
    """
    count = 0
    # Count from completed tricks
    for trick_list in self._completed_tricks:
      for (p, cardinfo) in trick_list:
        if p == player_id and cardinfo is not None:
          count += 1
    # Count from the in-progress trick
...
# Finally, register the game with PySpiel:
